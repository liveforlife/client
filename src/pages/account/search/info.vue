    ...
    <el-row type="flex" class="row-bg" justify="center">
      <el-form :model="ruleForm" ref="ruleForm" label-width="100px" class="demo-ruleForm" size="medium" status-icon>
        ...
        <template>
         <el-checkbox-group v-model="ruleForm.checkList">
          <el-col :span="9" v-for="(item, index) in arrRole" :key="index">
           <el-checkbox    :value="item.id"  :key="index" :label="item.id">{{item.name}}</el-checkbox>
          </el-col>
         </el-checkbox-group>
        </template>
       </el-form-item>
       ...
       <el-row type="flex" class="row-bg" justify="space-around" style="margin-top:60px;">
         ...
         <router-link to="/accountSearch" ><div class="save-btn liner-btn"><i class="icon-l icon-save"></i>返回</div></router-link>
       </el-row>
      </el-form>
    </el-row>
   </div>
...
<script>
 ...
 export default {
  ...
  methods: {
   getUserInfo () {
    if (this.query.account && this.query.company) {
     request.getUserInfo({
      userid: this.query.account,
      cid: this.query.company
     }).then(({ data }) => {
